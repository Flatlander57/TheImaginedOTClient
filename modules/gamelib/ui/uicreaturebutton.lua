...
function UICreatureButton:setup(creature)
  ...
  local shieldBarWidget = self:getChildById('shieldBar')
  ...
  self:setShieldBarPercent(creature:getShieldPercent())
...
function UICreatureButton:setShieldBarPercent(percent)
  local shieldBarWidget = self:getChildById('shieldBar')
  shieldBarWidget:setPercent(percent)

  local color = '#FFFFFF'

  shieldBarWidget:setBackgroundColor(color)
end